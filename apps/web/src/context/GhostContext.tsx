...
// One-time clean state reset to start all rounds, balances, and tickets at pure zero
if (typeof window !== 'undefined') {
  try {
    const version = localStorage.getItem('ghost_storage_v16_zero_tvl_clean');
    if (!version) {
      // Clear all historical round data, contaminated balances, and previous events
      const keysToRemove: string[] = [];
      for (let i = 0; i < localStorage.length; i++) {
        const k = localStorage.key(i);
        if (k && k.startsWith('ghost_') && !k.startsWith('ghost_accounts_') && !k.startsWith('ghost_current_user_email')) {
          keysToRemove.push(k);
        }
      }
      keysToRemove.forEach((k) => localStorage.removeItem(k));
      localStorage.setItem('ghost_storage_v16_zero_tvl_clean', 'active');
...
export const GhostProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Reactive total active deposits (TVL) across all savers in the protocol
  const totalPoolDeposits = useMemo(() => {
    let total = 0;
    try {
      for (let i = 0; i < localStorage.length; i++) {
        const k = localStorage.key(i);
        if (k && k.startsWith('ghost_balance_')) {
          const wallet = k.replace('ghost_balance_', '');
          if (address && wallet.toLowerCase() === address.toLowerCase()) {
            total += Math.max(0, userBalance);
          } else {
            const val = parseFloat(localStorage.getItem(k) || '0');
            if (val > 0) total += val;
          }
      ...
      if (address && userBalance > 0) {
        const hasKey = localStorage.getItem(`ghost_balance_${address.toLowerCase()}`);
        if (!hasKey) {
          total += userBalance;
        }
      }
  ...
  // Strict Zero-TVL Clamp: If there are no funds in the vault, lock the prize pool to 0.00 and stop all earning
  useEffect(() => {
  ...
  // Strict Zero-Balance Clamp: If user has 0 balance, clamp their personal accrued yield to 0
  useEffect(() => {
    if (userBalance <= 0) {
      setUserYield(0);
      if (address) {
        try {
          localStorage.setItem(`ghost_yield_${address.toLowerCase()}`, '0.00');
        } catch {
          // Ignore
        }
      }
  ...
  // Streams interest strictly when TVL > 0 (8.2% APY). Stops completely when TVL == 0.
  useEffect(() => {
    if (totalPoolDeposits <= 0) {
      setCurrentPrizePool(0);
      return;
    ...
    const catchUpPool = () => {
      ...
      if (totalPoolDeposits > 0 && elapsedSeconds > 0 && elapsedSeconds < 3600 * 24 * 7) {
        // 8.2% APY yield generated by all vault deposits: (totalDeposits * 0.082 * elapsedSeconds) / (365 * 86400)
        const catchUpYield = (totalPoolDeposits * 0.082 * elapsedSeconds) / (365 * 86400);
        if (catchUpYield > 0) {
    ...
    const poolInterval = setInterval(() => {
      ...
      if (totalPoolDeposits > 0) {
        const secondDrip = (totalPoolDeposits * 0.082) / (365 * 86400);
        const dripIncrement = Math.max(0.0001, +secondDrip.toFixed(5));
        setCurrentPrizePool((prev) => {
          const next = +(prev + dripIncrement).toFixed(4);
          try {
  ...
  useEffect(() => {
    ...
    const catchUpUserYield = () => {
      ...
      if (userBalance > 0 && elapsedSeconds > 0 && elapsedSeconds < 3600 * 24 * 30) {
