...
if (typeof window !== 'undefined') {
  try {
    const version = localStorage.getItem('ghost_storage_v14_pure_zero_baseline');
    if (!version) {
      ...
      localStorage.setItem('ghost_storage_v14_pure_zero_baseline', 'active');
...
export const GhostProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Streams interest strictly generated by active total deposits across all savers in the protocol (8.2% APY)
  useEffect(() => {
    ...
    const catchUpPool = () => {
      ...
      if (totalDeposits > 0 && elapsedSeconds > 0 && elapsedSeconds < 3600 * 24 * 7) {
        // 8.2% APY yield generated by all vault deposits: (totalDeposits * 0.082 * elapsedSeconds) / (365 * 86400)
        const catchUpYield = (totalDeposits * 0.082 * elapsedSeconds) / (365 * 86400);
        if (catchUpYield > 0) {
    ...
    const poolInterval = setInterval(() => {
      ...
      if (totalDeposits > 0) {
        const secondDrip = (totalDeposits * 0.082) / (365 * 86400);
